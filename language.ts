...
const enTranslation = {
    ...
    ButtonTextOpenLast: "Open last card in file", // Open the file location of the previous one
    ...
    SettingTextInitEaseDesc: "The base value for review frequency. The review interval is calculated as (this value/100) * time since the last review. A smaller number results in a higher review frequency. Recommended: 250.",
...
const zhTranslation = {
    ...
    SettingTextWaiting: "等待超时基准",
    SettingTextWaitingDesc: "等待超时有两个目的：1)在等待期间，它迫使你花时间复习、思考和记忆。2)更重要的是，它可以帮助您更准确地区分选项。你会发现，当你对答案有了一些想法时，如果你选择了错误的选项，你的惩罚时间会变得很长。因此，你更有可能选择合适的选项，而不是随机评估问题，因为这种评估非常不准确。例如，当您对答案不确定时，最好选择“不确定”选项。否则，如果你选择了“已知”选项，但答案是错误的，等待时间将会很长。您可以根据自己的情况调整此选项以延长或缩短等待超时的持续时间，也可以完全关闭等待。\n等待超时的推荐设置值，应该是您在三个小时内记住一个六个字母的单词并不忘记它的秒数。"
}
...
const twTranslation = {
    ...
    SettingTextWaiting: "等待超時基準",
    SettingTextWaitingDesc: "等待超時有兩個目的：1）在等待期間，它強迫你花時間複習、思考和記憶。2）更重要的是，它幫助你更準確地區分選項。你會發現，當你對答案有一些想法時，如果你選擇了錯誤的選項，懲罰時間會變得更長。因此，你更有可能選擇適當的選項，而不是隨機猜測問題，因為這種猜測非常不準確。例如，當你對答案不確定時，最好選擇「不確定」選項。否則，如果你選擇「我知道」選項但答案是錯誤的，等待時間會很長。你可以根據自己的情況調整該選項，延長或縮短等待超時的持續時間，或完全禁用它。推薦的等待超時值應該是你在三小時內記住一個六個字母的詞且不忘記的秒數。"
}
...
const koTranslation = {
    ...
    SettingTextWaiting: "대기 시간 제한 기준",
    SettingTextWaitingDesc: "대기 시간 제한은 두 가지 목적을 가지고 있습니다. 1) 대기 기간 동안, 복습, 고찰, 기억에 시간을 할애하도록 합니다. 2) 더 중요한 것은, 옵션을 보다 정확하게 구분할 수 있도록 도와줍니다. 답안에 대해 어느 정도 생각이 있을 때, 잘못된 옵션을 선택하면 처벌 시간이 길어집니다. 그래서 무작위로 추측하는 대신 올바른 옵션을 선택할 확률이 높아집니다. 예를 들어, 답안에 확신이 없을 때는 '확실하지 않아요' 옵션을 선택하는 것이 가장 좋습니다. 그렇지 않으면 '알아요' 옵션을 선택하지만 답안이 틀린 경우, 대기 시간이 길어집니다. 자신의 상황에 맞게 이 옵션을 조정하여 대기 시간의 기간을 연장하거나 축소할 수 있으며, 필요에 따라 완전히 비활성화할 수 있습니다. 대기 시간의 권장 값은 세 시간 내에 6글자 단어를 기억하고 잊지 않는 데 걸리는 시간입니다."
}
...
const itTranslation = {
    ...
    SettingTextWaiting: "Timeout di Attesa",
...
const plTranslation = {
    ...
    SettingTextWaiting: "Limit czasu oczekiwania",
...
const ukTranslation = {
    ...
    SettingTextWaiting: "Межа очікування",
...
const faTranslation = {
    ...
    SettingTextWaiting: "محدودیت انتظار",
